...
## 2024-07-15 - Debounce High-Frequency Synchronous LocalStorage Writes
**Learning:** Calling synchronous, blocking APIs like `localStorage.setItem` and `JSON.stringify` on high-frequency events (like `input` keystrokes) blocks the main thread and introduces lag.
**Action:** Always wrap high-frequency state persistence calls in a debounce function or `setTimeout`. This defers the expensive operation until the user pauses, significantly improving responsiveness and preventing unnecessary disk/storage I/O.
